...
const optimizeSwapAmounts = (paths, swapType, totalSwapAmount, initialSwapAmounts, highestLimitAmounts, inputDecimals, outputDecimals, initialNumPaths, maxPools, costReturnToken) => {
    ...
    let bestTotalReturnConsideringFees = swapType === SwapTypes.SwapExactIn ? INFINITY$1.times(-1) : INFINITY$1;
    let bestSwapAmounts = [];
    let bestPaths = [];
...
function setWrappedInfo(swapInfo, swapType, wrappedInfo, chainId) {
    ...
    if (wrappedInfo.tokenIn.wrapType === WrapTypes.ETH ||
        ...
        swapInfo.tokenAddresses = swapInfo.tokenAddresses.map((addr) => isSameAddress(addr, WETHADDR[chainId]) ? AddressZero : addr);
    }
...
function getLinearStaBal3Paths(tokenIn, tokenOut, poolsAllDict, poolsFilteredDict, chainId) {
    ...
    // If both tokenIn and tokenOut belong to linear pools
        ...
        if (!wethStaBal3Pool)
            return pathsUsingLinear;
        const wethTokenOutPath = getMostLiquidPath([staBal3Pool.address, WETHADDR[chainId], tokenOut], 1, [wethStaBal3Pool], poolsFilteredDict, SwapPairType.HopOut);
        ...
        if (!wethStaBal3Pool)
            return pathsUsingLinear;
        const tokenInWethPath = getMostLiquidPath([tokenIn, WETHADDR[chainId], staBal3Pool.address], 0, [wethStaBal3Pool], poolsFilteredDict, SwapPairType.HopIn);
...
class SOR {
    ...
    getSwaps(tokenIn, tokenOut, swapType, swapAmount, swapOptions) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const wrappedInfo = yield getWrappedInfo(this.provider, swapType, tokenIn, tokenOut, this.chainId, BigNumber$1.from(swapAmount));
            let swapInfo;
